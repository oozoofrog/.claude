...
command_not_found_handler () {
	if [[ "$1" != "mise" && "$1" != "mise-"* ]] && /Users/oozoofrog/.local/bin/mise hook-not-found -s zsh -- "$1"
	then
		_mise_hook
		"$@"
...
mise () {
	...
	if [ "$#" = 0 ]
	then
		command /Users/oozoofrog/.local/bin/mise
		return
	fi
	shift
	case "$command" in
		(deactivate | shell | sh) if [[ ! " $@ " =~ " --help " ]] && [[ ! " $@ " =~ " -h " ]]
			then
				eval "$(command /Users/oozoofrog/.local/bin/mise "$command" "$@")"
				return $?
			fi ;;
	esac
	command /Users/oozoofrog/.local/bin/mise "$command" "$@"
}
mkcd () {
	mkdir -p $@ && cd ${@:$#}
}
...
export PATH=/opt/homebrew/bin\:/Users/oozoofrog/.nvm/versions/node/v18.17.0/bin\:/opt/homebrew/bin\:/opt/homebrew/sbin\:/usr/local/bin\:/System/Cryptexes/App/usr/bin\:/usr/bin\:/bin\:/usr/sbin\:/sbin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin\:/Library/Apple/usr/bin\:/Users/oozoofrog/.nvm/versions/node/v18.17.0/bin\:/Users/oozoofrog/.local/bin\:/Users/oozoofrog/.cargo/bin\:/Users/oozoofrog/.cursor/extensions/ms-python.debugpy-2025.8.0-darwin-arm64/bundled/scripts/noConfigScripts
