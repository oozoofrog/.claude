...
Leverage Gemini CLI for complex task management and analysis:

### Usage Scenarios
1. **Complete Project Analysis**
   ```bash
   # Complete project scope understanding
   gemini -p "Analyze all project components and suggest task hierarchy" . --exclude "node_modules,dist"
   
   # Task complexity evaluation
   gemini -p "Evaluate complexity of each module and set task priorities" src/
   ```

2. **Resource and Dependency Analysis**
   ```bash
   # Dependency mapping
   gemini -p "Analyze all dependencies in the project and optimize task order"
   
   # Bottleneck prediction
   gemini -p "Analyze expected bottlenecks and risks during task execution"
   ```

3. **Performance Analysis and Optimization**
   ```bash
   # Task pattern analysis
   gemini -p "Analyze past task history and suggest productivity improvements" work-logs/
   
   # Quality metrics analysis
   gemini -p "Analyze task quality metrics and find improvement points"
   ```

### Collaboration Strategy
- **Strategy Planning**: Use Gemini to analyze complex tasks and establish strategies
- **Execution Management**: Use Claude Code to perform accurate task execution and tracking
- **Performance Analysis**: Analyze task results and continuous improvement